function [frameTimes, frameTimesoffset, frameInd] = detectViStim_MS(dataFilename, nChans, sr)
...
% Detect frame times
displacement = 1;
threshold = max(NIDQ) - 0.5*int16(std(single(NIDQ)));
%threshold = mode(NIDQ) + 0.5*int16(std(single(NIDQ))); % threshold based on mode + 0.5SD - only works if presentation time is shorter than inter trial interval
